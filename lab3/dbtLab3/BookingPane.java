...
public class BookingPane extends BasicPane {
    ...
    private void fillNameList() {
        ...
        for (Movie m : db.getMovies()){
            ...
            nameListModel.addElement(m.title);
        }
    }
    ...
    private void fillDateList(String movie) {
        ...
        for(MoviePreformance mp : db.getMoviePreformances(movie)){
            dateListModel.addElement(mp.date.toString());
        }
    }
    ...
    class DateSelectionListener implements ListSelectionListener {
        ...
        public void valueChanged(ListSelectionEvent e) {
            ...
            /* --- insert own code here --- */
            for (MoviePreformance m: db.getMoviePreformances(movieName, date)){
                fields[0].setText(m.title);
                fields[1].setText(m.date);
                fields[2].setText(m.name);
                fields[3].setText(m.booked_seats.toString());
            }
    ...
    class ActionHandler implements ActionListener {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            String date = dateList.getSelectedValue();
            /* --- insert own code here --- */
            int n = db.updateMoviePreformance(movieName, date);
            if (n != 0) {
                db.bookTicket(movieName, date, CurrentUser.instance().getCurrentUserId());
                JOptionPane.showMessageDialog(null, "Booking successful");
            } else {
                JOptionPane.showMessageDialog(null, "Bokking unsuccessful");
            }
        }
    }
}
